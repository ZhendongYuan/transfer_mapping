...
print("running coral rtm2ams ufp all roads")
...
model = CORAL(Ridge(alpha=0.8), lambda_= 1e-1, Xt=Xt, random_state=0)
model.fit(Xs, ys)
...
pd.DataFrame(yval_pred).to_csv(
            "save the predictions at validation sites")